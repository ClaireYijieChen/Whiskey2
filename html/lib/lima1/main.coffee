...
class DataManager
  ...
  removeCascade: (stream, object, handler) ->
    ...
    gr = new AsyncGrouper selects.length, (gr) =>
      ...
      if err then return handler err
      config = []
      config.push type: 'remove', stream: stream, object: object
      for i in [0...gr.results.length]
        arr = gr.results[i]
        for data in arr[0]
          config.push type: 'removeCascade', stream: selects[i].stream, object: data
      # log 'Execute batch', config
      @batch config, handler
    # log 'selects', selects
    for select in selects
      @storage.select select.stream, select.query, gr.fn

  batch: (config = [], handler) ->
    ag = new AsyncGrouper config.length, (ag) =>
      err = ag.findError()
      result = []
      for arr in ag.results
        result.push arr[0]
      if handler then handler err, result
    for item in config
      switch item.type
        when 'create' then @storage.create item.stream, item.object, ag.fn, item.options
        when 'update' then @storage.update item.stream, item.object, ag.fn, item.options
        when 'remove' then @storage.remove item.stream, item.object, ag.fn, item.options
        when 'removeCascade' then @removeCascade item.stream, item.object, ag.fn, item.options
        when 'findOne' then @findOne item.stream, item.id, ag.fn
        else
          ag._fn null
